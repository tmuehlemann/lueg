...
import {
  generateMetaColors,
  type MetaColors,
} from "$lib/server/metadata/metaColors";
import tinycolor from "tinycolor2";
import { resizeImage } from "$lib/server/metadata/process_images/resizeImage";

export async function publishMovie(fileUploadId: number, tmdbId: number) {
  // get file upload
  const movieFileUpload = await db.query.fileUpload.findFirst({
    where: (fileUpload, { and, eq }) =>
      and(eq(fileUpload.id, fileUploadId), isNull(fileUpload.movieId)),
  });

  if (!fileUpload) {
    throw new Error(
      "File upload not found, because the id is wrong or it is already published",
    );
  }
  ...
  try {
    ...
    await downloadFile(
      TMDB_BASE + metadata.poster_path,
      "static/metadata/posters/" + filename + ".jpg",
    );
    await downloadFile(
      TMDB_BASE + metadata.backdrop_path,
      "static/metadata/backdrops/" + filename + ".jpg",
    );
    ...
    // resized images
    await resizeImage({
      originalPath: "static/metadata/posters/" + filename + ".jpg",
      width: 200,
      height: 300,
    });
  } catch (e) {
...
async function createMovie(file: typeof fileUpload, metadata: TmdbMovie) {
  ...
  try {
    ...
    movieColors = await generateMetaColors(
  ...
  await db.transaction(async (t1) => {
    ...
    await t1.insert(movieToGenre).values(
